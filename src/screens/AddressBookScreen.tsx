...
export const AddressBookScreen = ({ t, contacts, onSave, notify, onBack }: any) => {
  ...
  const addContact = () => {
    ...
    try {
      ...
      notify(t('added') || '追加しました');
    } catch(e) {
      setAlert({ visible: true, title: t('error') || 'エラー', message: t('invalid_address') || '無効なアドレスです', type: 'error' });
    }
  };
  ...
  const deleteContact = (index: number) => {
    setConfirm({
      visible: true,
      title: t('delete') || '削除',
      message: t('confirm_delete') || '本当に削除しますか？',
      onConfirm: () => {
  ...
  const handleScan = (scannedValue: string) => {
    ...
    notify(t('qr_scanned') || 'スキャン成功！');
  };

  return (
    <View style={{ flex: 1, backgroundColor: 'transparent' }}>
      <HeaderRow 
        title={t('address_book') || 'アドレス帳'} 
        onBack={onBack} 
        rightIcon={
      ...
      >
        {/* 🌟 修正: Empty を t() で囲んだ */}
        {contacts.length === 0 ? <Text style={styles.descText}>{t('empty') || 'Empty'}</Text> : contacts.map((c: any, i: number) => (
          <View key={i} style={styles.settingItem}>
             ...
             <View style={{flexDirection:'row', gap:15}}>
               <TouchableOpacity onPress={() => {Clipboard.setString(c.address); notify(t('copied') || 'コピーしました');}}>
      ...
      {modalVisible && (
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "padding"} style={styles.modalOverlay}>
          <View style={[styles.modalContent, { height: 'auto', paddingBottom: 40 }]}>
            <Text style={styles.sectionTitle}>{t('add_new') || '新規追加'}</Text>
            
            <TextInput 
              style={styles.inputField} 
              placeholder={t('name_placeholder') || '名前'} 
            ...
            <View style={{ position: 'relative', width: '100%', marginBottom: 16 }}>
              <TextInput 
                ...
                placeholder={t('address_placeholder') || 'アドレス'} 
                placeholderTextColor="#666" 
                value={newAddr} 
            ...
            <TouchableOpacity style={styles.primaryButton} onPress={addContact}>
              <Text style={styles.primaryButtonText}>{t('save') || '保存'}</Text>
            </TouchableOpacity>
            
            <TouchableOpacity onPress={() => setModalVisible(false)} style={{ marginTop: 16, paddingVertical: 10 }}>
              <Text style={{ color: '#666', textAlign: 'center', fontWeight: 'bold' }}>{t('close') || '閉じる'}</Text>
            </TouchableOpacity>
          </View>
      ...
      <ConfirmModal visible={confirm.visible} title={confirm.title} message={confirm.message} confirmText={t('delete') || '削除'} cancelText={t('cancel') || 'キャンセル'} onCancel={() => setConfirm({ ...confirm, visible: false })} onConfirm={confirm.onConfirm} />
