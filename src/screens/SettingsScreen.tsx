  ...
  return (
    <SafeAreaView style={globalStyles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: showBanner ? BANNER_ESTIMATED_HEIGHT + 40 : 60 }}>
        ...
        <Text style={globalStyles.sectionTitle}>{t('settings_general') || 'General'}</Text>
        <View style={globalStyles.card}>
          <SettingItem icon={Users} title={t('address_book') || 'Address Book'} desc={t('settings_address_book_desc') || 'Manage saved addresses'} onPress={() => onNavigate('address_book')} color="rgba(59, 130, 246, 0.1)" iconColor="#3b82f6" />
          <SettingItem icon={ShieldCheck} title={t('settings_security') || 'Security & Privacy'} desc={t('settings_security_desc') || 'PIN & Biometrics'} onPress={() => onNavigate('settings_security')} color="rgba(34, 197, 94, 0.1)" iconColor="#22c55e" />
          <SettingItem icon={Server} title={t('settings_network') || 'Network Settings'} desc={t('settings_network_desc') || 'Mainnet / Devnet, RPC'} onPress={() => onNavigate('settings_network')} color="rgba(245, 158, 11, 0.1)" iconColor="#f59e0b" />
          <SettingItem icon={Globe} title={t('settings_language') || 'Language'} desc={t('settings_language_desc') || 'Change app language'} onPress={() => onNavigate('settings_lang')} color="rgba(236, 72, 153, 0.1)" iconColor="#ec4899" isLast />
        </View>

        <Text style={globalStyles.sectionTitle}>{t('settings_support') || 'Support & About'}</Text>
        <View style={globalStyles.card}>
          <SettingItem icon={CircleHelp} title={t('settings_help') || 'Help Center'} desc={t('settings_help_desc') || 'FAQ & Guides'} onPress={() => onNavigate('settings_help')} color="rgba(14, 165, 233, 0.1)" iconColor="#0ea5e9" />
          <SettingItem icon={Info} title={t('settings_about') || 'About App'} desc={`Version ${packageJson.version}`} onPress={() => onNavigate('settings_about')} color="rgba(168, 85, 247, 0.1)" iconColor="#a855f7" isLast />
        </View>

        <Text style={globalStyles.sectionTitle}>{t('settings_danger') || 'Danger Zone'}</Text>
        <View style={globalStyles.card}>
          <SettingItem icon={LogOut} title={t('logout_btn') || 'Log Out'} desc={t('settings_logout_desc') || 'Remove wallet from this device'} onPress={onLogout} isDanger isLast />
        </View>
